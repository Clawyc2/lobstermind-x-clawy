...
export default {
  ...
  configSchema: {
    ...
    properties: {
      ...
      embeddingProvider: {
        type: 'string',
        enum: ['gemini', 'openai', 'ollama', 'transformers', 'sha256'],
        default: 'openai',
        description: 'Proveedor de embeddings: openai (API), gemini (API), ollama (local), transformers (local), sha256 (fallback)'
      },
      ...
      openaiApiKey: {
        type: 'string',
        default: '',
        description: 'API key de OpenAI (requerido si usás openai provider). Por defecto usa OPENAI_API_KEY env var.'
      },
      openaiModel: {
        type: 'string',
        default: 'text-embedding-3-small',
        description: 'Modelo de OpenAI para embeddings (ej: text-embedding-3-small, text-embedding-3-large)'
      },
      geminiModel: {
        type: 'string',
  ...
  async register(api: any) {
    ...
    const validateConfig = async () => {
      ...
      // Validar provider
      const validProviders = ['openai', 'gemini', 'ollama', 'transformers', 'sha256'];
      if (!validProviders.includes(embeddingProvider)) {
        errors.push(`Provider "${embeddingProvider}" no es válido. Usá: ${validProviders.join(', ')}`);
      }

      // Validar OpenAI / OpenRouter
      if (embeddingProvider === 'openai') {
        const apiKey = config.openaiApiKey || process.env.OPENAI_API_KEY || process.env.OPENROUTER_API_KEY;
        if (!apiKey) {
          errors.push('OpenAI provider requiere openaiApiKey en config, OPENAI_API_KEY o OPENROUTER_API_KEY env var');
        } else {
          try {
            const baseUrl = config.openaiBaseUrl || process.env.OPENAI_BASE_URL || 'https://openrouter.ai/api/v1';
            const model = config.openaiModel || 'text-embedding-3-small';
            const response = await fetch(`${baseUrl}/embeddings`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}`, 'HTTP-Referer': 'https://clawyc2.github.io' },
              body: JSON.stringify({ model, input: 'test' })
            });
            if (!response.ok) {
              warnings.push(`OpenAI/Router API respondió con ${response.status}. Verificá tu API key.`);
            }
          } catch (e: any) {
            warnings.push(`No se pudo conectar a OpenAI/Router API: ${e.message}`);
          }
        }
      }
    ...
    const ws = api.runtime?.workspace || '/home/ubuntu/.openclaw/workspace';
    if (!api.runtime?.workspace) {
      console.warn('[lobstermind] No workspace from API, using fallback:', ws);
    }
    ...
    // embeddingProvider ya declarado arriba en validateConfig
    ...
    const embedWithOpenAI = async (text: string): Promise<number[]> => {
      const startTime = Date.now();
      const apiKey = config.openaiApiKey || process.env.OPENAI_API_KEY || process.env.OPENROUTER_API_KEY;

      if (!apiKey) {
        throw new Error('OpenAI API key no configurada. Seteá openaiApiKey en config o OPENAI_API_KEY env var.');
      }
      ...
      try {
        ...
        if (!response.ok) {
          const error = await response.text();
          throw new Error(`OpenAI API error (${response.status}): ${error.substring(0, 100)}`);
        }
        ...
        if (!embedding || !Array.isArray(embedding)) {
          throw new Error('OpenAI no retornó embedding válido');
        }

        const latency = Date.now() - startTime;
        updateEmbeddingStats(latency);

        console.log(`[lobstermind:embed] OpenAI (${model}): ${embedding.length} dims en ${latency}ms`);
        return embedding;
    ...
    const embed = async (text: string, useCache: boolean = true): Promise<number[]> => {
      ...
      try {
        ...
        switch (embeddingProvider) {
          case 'openai':
            embedding = await embedWithOpenAI(text);
            break;
          case 'gemini':
      ...
      if (config.nativeSyncEnabled !== false) {
        try {
          ...
          if (existsSync(nativePath)) {
            nativeContent = readFileSync(nativePath, 'utf-8');
          } else {
            writeFileSync(nativePath, '# Memories\n\nAuto-created by LobsterMind Memory plugin\n\n', 'utf-8');
            nativeContent = '';
          }
          if (!nativeContent.includes(nativeEntry.trim())) {
    ...
    // NUDGE: acumular mensajes y clasificar cada NUDGE_INTERVAL turnos
    ...
    // ========================================================================
    // AUTO-CAPTURE V2: LLM Classification + Supabase sync
    // ========================================================================

    // Load env vars for OpenRouter + Supabase
    const loadEnvVars = () => {
      try {
        const envFile = require('fs').readFileSync('/home/ubuntu/.config/clawy/.env', 'utf-8');
    ...
    const LLM_PRIMARY = 'stepfun/step-3.5-flash:free';
    const LLM_FALLBACK = 'minimax/minimax-m2.5:free';
    const CAPTURE_MIN_CHARS = 30;

    if (OPENROUTER_KEY && SUPABASE_URL) {
      console.log(`[lobstermind:ac] Auto-capture V2 ready — OpenRouter + Supabase, nudge every ${NUDGE_INTERVAL} turns`);
    } else {
      console.log(`[lobstermind:ac] ⚠️ Missing OPENROUTER_KEY or SUPABASE_URL — auto-capture V2 disabled (regex fallback active)`);
    }

    const CLASSIFY_PROMPT = `Analiza este fragmento de conversación entre un usuario (Luis) y su asistente de IA (Clawy).

Responde SOLO con un JSON array. Si nada es digno de recordar, responde [].
Tipos válidos: ERROR, REGLA, LECCION, PREFERENCIA, PATRON, DECISION, PROYECTO

Reglas:
- NO guardar saludos, confirmaciones ("sí", "ok"), preguntas simples
- NO guardar conversación casual
- Guardar SOLO si contiene algo con valor duradero
- El contenido debe ser conciso (máximo 200 chars)
- En español siempre

Formato exacto:
[{"type":"ERROR","content":"descripción corta"},{"type":"REGLA","content":"descripción corta"}]`;

    async function classifyWithLLM(text: string): Promise<Array<{type: string, content: string}>> {
      if (!text || text.length < CAPTURE_MIN_CHARS) return [];
      for (const model of [LLM_PRIMARY, LLM_FALLBACK]) {
        try {
          ...
          console.log(`[lobstermind:ac] LLM ${model} failed:`, e.message);
        }
      }
      return [];
    }

    async function saveToSupabase(memories: Array<{type: string, content: string}>): Promise<number> {
      let saved = 0;
      for (const mem of memories) {
        try {
          // Dedup check via Supabase
          const checkRes = await fetch(`${SUPABASE_URL}/rest/v1/autoaprendizaje?select=id,content&content=ilike.%25${encodeURIComponent(mem.content.slice(0, 30))}%25&limit=1`, {
            headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` },
          });
          const existing = await checkRes.json();
          if (Array.isArray(existing) && existing.length > 0) {
            console.log(`[lobstermind:ac] Dedup: "${mem.content.slice(0, 40)}..." ya existe en Supabase`);
            continue;
          }

          await fetch(`${SUPABASE_URL}/rest/v1/autoaprendizaje`, {
            method: 'POST',
          ...
          console.log(`[lobstermind:ac] Supabase save error:`, e.message);
        }
      }
      return saved;
    }
    ...
    if (api.hooks?.afterMessage) {
      api.hooks.afterMessage((message: any) => {
        console.log(`[lobstermind:debug] afterMessage FIRED — role: ${message?.role}, content_type: ${typeof message?.content}, len: ${typeof message?.content === 'string' ? message.content.length : 'N/A'}`);
    ...
    // ========================================================================
    // HOOKS DE OPENCLAW - message_sent (auto-capture)
    // ========================================================================

    if (api.on) {
      api.on('message_sent', (event: any, ctx: any) => {
        const message = ctx?.message || event?.message;
        if (message?.role === 'assistant' && message?.content) {
          ...
          if (turnCounter >= NUDGE_INTERVAL && pendingMessages.length > 0) {
            const batchToProcess = [...pendingMessages];
            pendingMessages.length = 0;
            turnCounter = 0;

            // Procesar batch: buscar contenido valioso en los últimos N turnos
            (async () => {
              try {
                // Filtrar mensajes cortos y triviales primero
                const filtered = batchToProcess.filter(msg => {
                  const trimmed = msg.trim();
                  return trimmed.length >= 10
                    && !trimmed.endsWith('?')
                    && !/^(hi|hola|gracias|thanks|ok|sure|NO_REPLY)$/i.test(trimmed);
                });

                if (filtered.length === 0) {
                  autoCaptureStats.trueNegatives += batchToProcess.length;
                  return;
                }

                // Tomar los mensajes más largos/valiosos del batch (top 3)
                const sorted = filtered.sort((a, b) => b.length - a.length);
                const topMessages = sorted.slice(0, 3);
                const turnText = topMessages.join('\n---\n');

                if (OPENROUTER_KEY && SUPABASE_URL) {
                  // V2: LLM classification via OpenRouter + Supabase sync
                  const classified = await classifyWithLLM(turnText);
                  if (classified.length > 0) {
                    const supaSaved = await saveToSupabase(classified);
                    for (const mem of classified) {
                      await save(mem.content, mem.type, mem.type === 'ERROR' || mem.type === 'REGLA' ? 0.95 : 0.85);
                    }
                    autoCaptureStats.totalCaptured += classified.length;
                    autoCaptureStats.truePositives += classified.length;
                    autoCaptureStats.lastCaptureTime = new Date().toISOString();
                    console.log(`[lobstermind:ac] ✅ Captured ${classified.length} memories (${supaSaved} to Supabase): ${classified.map(m => m.type).join(', ')}`);
                  } else {
                    autoCaptureStats.falseNegatives += topMessages.length;
                    console.log(`[lobstermind:ac] Nothing worthy in batch of ${batchToProcess.length} messages`);
                  }
                } else {
                  // Fallback: regex classification (local only)
                  for (const msg of topMessages) {
                    autoCaptureStats.totalProcessed++;
                    const classified = classifyMemoryContent(msg);
                    if (classified.shouldSave) {
                      await save(classified.content, classified.type, classified.confidence);
                      autoCaptureStats.totalCaptured++;
                      autoCaptureStats.truePositives++;
                ...
                autoCaptureStats.trueNegatives += (batchToProcess.length - topMessages.length);
              } catch (e: any) {
                console.error('[lobstermind:nudge] Error processing batch:', e.message);
              }
            })();
          }
        }
      });
      console.log('[lobstermind] Registered hook: message_sent (auto-capture, nudge every ' + NUDGE_INTERVAL + ' turns)');
    }

    // ========================================================================
    // AUTO-INJECT INTELIGENTE: autoaprendizaje + sesiones recientes
    // ========================================================================

    const loadAutoaprendizaje = (mode: 'bootstrap' | 'top'): any[] => {
      try {
        if (mode === 'bootstrap') {
          const errors = db.prepare("SELECT content, category FROM autoaprendizaje WHERE category = 'error' ORDER BY importance DESC, updated_at DESC LIMIT 40").all() as any[];
          const rules = db.prepare("SELECT content, category FROM autoaprendizaje WHERE category = 'regla' ORDER BY importance DESC, updated_at DESC LIMIT 40").all() as any[];
          return [...errors, ...rules].filter(i => i.content && i.content.length > 10);
        } else {
          const cats = ['error', 'regla', 'leccion'];
          const results: any[] = [];
          for (const cat of cats) {
            const items = db.prepare(`SELECT content, category FROM autoaprendizaje WHERE category = ? ORDER BY importance DESC, updated_at DESC LIMIT 3`).all(cat) as any[];
            results.push(...items.filter(i => i.content && i.content.length > 10));
          }
          return results;
        }
      } catch (e: any) {
        console.error('[lobstermind:inject] Error loading autoaprendizaje:', e.message);
        return [];
      }
    };

    const formatInjectContext = (autoItems: any[], semanticResults: any[], recentChunks: any[] = []): string => {
      const parts: string[] = [];

      if (recentChunks.length > 0) {
        const lines = recentChunks.map((c: any, i: number) => {
          const session = (c.session_id || '').replace('txt_sess_', '').slice(0, 15);
          return `${i + 1}. [sesión ${session}] ${c.content.slice(0, 500)}`;
        });
        parts.push(`<recent-sessions>\n### ÚLTIMAS 5 SESIONES (último chunk de cada una):\n${lines.join('\n---\n')}\n</recent-sessions>`);
      }

      if (autoItems.length > 0) {
        const lines = autoItems.map((a: any) => `- [${(a.category || '').toUpperCase()}] ${a.content}`);
        parts.push(`<clawy-autoaprendizaje>\n${lines.join('\n')}\n</clawy-autoaprendizaje>`);
      }

      if (semanticResults.length > 0) {
        const lines = semanticResults.map((m: any, i: number) => `${i + 1}. [${m.type}] ${m.content} (score: ${m.score.toFixed(3)})`);
    ...
    if (api.registerCli) {
      api.registerCli(({program}: any) => {
        ...
        c.command('sync-obsidian').action(async () => {
          ...
          const obs = join(obsidianDir, 'Memories.md');
          if (!existsSync(obs)) {
            console.log('i️  No existe archivo de Obsidian para sync');
            return;
          }
